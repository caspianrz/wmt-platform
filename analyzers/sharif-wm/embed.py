...
def embed(image, watermark):
    try:
        ...
        img_pt = util.load_img_from_base64(image)
        output = wam.embed(img_pt, wm_msg)
        mask = util.create_random_mask(img_pt, num_masks=1, mask_percentage=proportion_masked)
        img_w = output['imgs_w'] * mask + img_pt * (1 - mask)
        return util.tensor_to_base64(unnormalize_img(img_w))
    except Exception as e:
        print(e)

if __name__ == "__main__":
    embed_path(sys.argv[1], sys.argv[2], sys.argv[3])
