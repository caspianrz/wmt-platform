...
const applyWatermarkHandler = async (req: Request, res: Response) => {
  ...
  const imagePath = path.join(
    uploadDir,
    `/${aReq.user}/assets/${wmData.data.image}`
  );
  const wmRecord = await DatabaseManager.instance.getWatermark(
    wmData.data.watermark
  );
  const watermarkPath = wmRecord.path;
  const outputuuid = uuid.v4();
  const outputDir = path.join(
    uploadDir,
    `/${aReq.user}/watermarked/${outputuuid}`
  );
  ...
  switch (wmData.data.strategy) {
    ...
    default:
      return res
        .status(400)
        .json({ message: `${wmData.data.strategy} strategy not found.` });
  }
};
