...
export function meta({ }: Route.MetaArgs) {
	return [
		{ title: "Analyze" },
		{ name: "description", content: "Analyze Watermarked image." },
	];
}
...
export default function AnalyzePage() {
	...
	const [watermarkedImage, setWatermarkdImage] = useState<string | undefined>(undefined);
	const [exWat, setExWat] = useState<string | undefined>(undefined);
	useEffect(() => {
		if (auth.user != null) {
			const actual = EnvironmentManager.Instance.endpoint(`/api/uploads/${auth.user}/watermarked/${uuid}/${kind}-0`).href;
			setWatermarkdImage(actual);
			const extracted = EnvironmentManager.Instance.endpoint(`/api/uploads/${auth.user}/watermarked/${uuid}/out`).href;
			setExWat(extracted);
		}
	}, [auth.user]);
